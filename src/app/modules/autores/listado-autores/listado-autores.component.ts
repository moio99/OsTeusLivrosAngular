...
import { InformacomPeTipo, ListadosAutoresTipos } from 'src/app/shared/enums/estadisticasTipos';
import { ListadoAutores, ListadoAutoresData, Parametros } from './listado-autores.interface';
  ...
  private obterDadosDoListadoPorTipo(parametros: Parametros): void {
    if (parametros.tipo == ListadosAutoresTipos.porNacionalidade)
    {
      this.outrosService
        ...
        .pipe(first())
        .subscribe({
          next: (v) => this.filtroPaisOuNacionalidade = ' ' + v,
          error: (e) => { console.error(e),
