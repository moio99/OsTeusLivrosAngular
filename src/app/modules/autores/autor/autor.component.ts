...
import { Validators, ValidatorFn, FormBuilder, AbstractControl, ValidationErrors, FormControl } from '@angular/forms';
import { ActivatedRoute, Router } from '@angular/router';
import { first, map, Observable, startWith } from 'rxjs';
import { AutoresService } from 'src/app/core/services/api/autores.service';
...
import { DadosObtidos, Nacionalidade, Pais, SimpleObjet, } from 'src/app/shared/models/outros';
import { ListadoLivros, ListadoLivrosData } from '../../livros/listado-livros/listado-livros.interface';
import { Autor, Parametros, AutorData } from './autor.interface';
...
export class AutorComponent implements OnInit {
  ...
  private obterLivros(id: number): void {
    this.livrosService
      ...
      .pipe(first())
      .subscribe({
        next: (v) => this.dadosLivrosDoAutor = this.dadosLivrosObtidos(v),
  ...
  private obterNacionalidades(idAutor: number): void {
    this.outrosService
      .getNacionalidades()
      .pipe(first())
      .subscribe({
        next: (v) => this.dadosNacionalidades = this.dadosNacionalidadesObtidas(v),
        error: (e) => { console.error(e),
  ...
  private dadosNacionalidadesObtidas(data: object): Nacionalidade[] {
    ...
    if (dados != null && dados.data.length > 0) {
      ...
      this.dadosNacionalidadesFiltradas = this.af.nacom.valueChanges
        .pipe(
          startWith(''),
          map(value => this.filtroDeNacionalidades(<string>value))
        );
  ...
  */
  ...
  private obterPaises(idAutor: number): void {
    this.outrosService
      .getPaises()
      .pipe(first())
      .subscribe({
        next: (v) => this.dadosPaises = this.dadosPaisesObtidos(v),
        error: (e) => { console.error(e),
  ...
  private dadosPaisesObtidos(data: object): Pais[] {
    ...
    if (dados != null && dados.data.length > 0) {
      ...
      this.dadosPaisesFiltrados = this.af.pais.valueChanges
        .pipe(
          startWith(''),
          map(value => this.filtroDePaises(<string>value))
        );
  ...
  */
  ...
  private obterDadosDoAutor(id: number): void {
    if (id > 0) {
      this.autoresService
        .getAutor(id)
        .pipe(first())
    ...
    if (autorRepetido != undefined && autorRepetido.meta.quantidade > 0 && (
      ...
      if (event.submitter.value === this.engadir) {
        this.autoresService
          ...
          .pipe(first())
          .subscribe({
            next: (v) => {console.debug(v), this.gestionarRetroceso(v, autor)},
            error: (e) => {
